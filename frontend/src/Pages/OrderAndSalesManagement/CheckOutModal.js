...
const CheckOutModal = ({ isOpen, onOpenChange, total }) => {
  ...
  const validate = () => {
    const newErrors = {};
    if (!name) newErrors.name = "Name is requird";
    if (!name.length < 2) newErrors.name = "Name must be more than 2 letters";
    if (!email) newErrors.email = "Email is required";
    if (!city) newErrors.city = "City is required";
    if (!city.length < 2) newErrors.city = "City must be more than 2 letters";
    if (!phone) newErrors.phone = "Phone number is required";
    if (!address) newErrors.address = "Address is required";
    if (!address.length < 5) newErrors.address = "Address must be more than 5 letters";
    if (!/^\S+@\S+\.\S+$/.test(email)) newErrors.email = "Email is invalid";
    if (!/^\d{10,15}$/.test(phone)) newErrors.phone = "Phone number is invalid";
